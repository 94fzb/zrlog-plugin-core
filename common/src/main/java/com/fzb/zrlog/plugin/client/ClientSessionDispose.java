...
public class ClientSessionDispose implements ISessionDispose {
    ...
    private static Logger LOGGER = LoggerUtil.getLogger(ClientSessionDispose.class);

    @Override
    public void handler(IOSession session, MsgPacket msgPacket) {
        ...
        if (action == ActionType.HTTP_FILE) {
            ...
            if (in != null) {
                ...
                session.sendMsg(ContentType.BYTE, tmpBytes, action.name(), msgPacket.getMsgId(), MsgPacketStatus.RESPONSE_SUCCESS, null);
            } else {
                session.sendMsg(ContentType.BYTE, new byte[]{}, action.name(), msgPacket.getMsgId(), MsgPacketStatus.RESPONSE_ERROR, null);
            }
